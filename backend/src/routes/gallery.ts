  ...
  app.post("/gallery", async (req, reply) => {
    try {
      ...
      if (type === "external") {
        ...
        if (!imageUrl) return reply.status(400).send({ error: "imageUrl is required for internal images" });
    ...
    try {
      ...
      if (event.type === "external") {
        ...
        if (nextSlug !== event.slug) {
          ...
          if (slugExists && slugExists.id !== id) return reply.status(409).send({ error: "Slug already exists" });
        }
        ...
        if (body?.galleryImages !== undefined) {
          ...
          const images = mapImages(body.galleryImages);
          if (images.length) await imageRepo.save(images.map((item: any) => ({ ...item, eventId: event.id })));
        }
      } else {
