...
export default async function galleryRoutes(app: FastifyInstance) {
  ...
  const mapImages = (value: any[]) =>
    Array.isArray(value)
      ? value
          ...
          .filter((item: any) => item.imageUrl.length > 0)
      : [];
  ...
  app.get<{ Params: { id: string } }>("/gallery/:id", async (req, reply) => {
    try {
      ...
      if (!event) return reply.status(404).send({ error: "Gallery event not found" });
  ...
  app.post("/gallery", async (req, reply) => {
    try {
      ...
      if (images.length) {
        await imageRepo.save(images.map((item: any) => ({ ...item, eventId: saved.id })));
      }
      ...
      app.log.error(error);
      return reply.status(500).send({ error: "Failed to create gallery event" });
    }
  });

  app.put<{ Params: { id: string } }>("/gallery/:id", async (req, reply) => {
    try {
      const id = Number(req.params.id);
      const body = req.body as any;
      ...
      event.slug = nextSlug;
      if (body?.location !== undefined) event.location = body.location ? String(body.location) : null;
      if (body?.coverImage !== undefined) event.coverImage = body.coverImage ? String(body.coverImage) : null;
      if (body?.description !== undefined) event.description = body.description ? String(body.description) : null;
      ...
      if (body?.galleryImages !== undefined) {
        ...
        const images = mapImages(body.galleryImages);
        if (images.length) {
          await imageRepo.save(images.map((item: any) => ({ ...item, eventId: event.id })));
        }
      }
      ...
      app.log.error(error);
      return reply.status(500).send({ error: "Failed to update gallery event" });
    }
  });

  app.delete<{ Params: { id: string } }>("/gallery/:id", async (req, reply) => {
    try {
      const id = Number(req.params.id);
      ...
      app.log.error(error);
      return reply.status(500).send({ error: "Failed to delete gallery event" });
    }
  });
}
