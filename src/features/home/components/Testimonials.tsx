...
import { ApiTestimonial, getInitials } from "@/features/home/types/testimonial";
...
export default function Testimonials({ items }: TestimonialsProps) {
    ...
    return (
      <section className="w-full bg-[#0FA4AF] py-[50px] md:py-[80px] relative overflow-hidden px-4">
        <div className="max-w-[1200px] mx-auto text-center text-white">
          ...
          <p className="mt-[12px] text-[14px] md:text-[16px] opacity-75 italic">
  ...
  return (
    <section
      className="w-full bg-[#0FA4AF] py-[50px] md:py-[80px] relative overflow-hidden px-4"
    ...
    >
      <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row items-center gap-[30px] md:gap-[40px]">
        ...
        <div className="md:w-[380px] text-white text-center md:text-left">
          ...
          {/* Dot navigation */}
          <div className="flex gap-[8px] mt-[20px] md:mt-[28px] justify-center md:justify-start">
            {items.map((_, i) => (
              <button
                key={i}
...
/* ── Card ── */
function Card({ item }: { item: ApiTestimonial }) {
  const initials = getInitials(item.name);
  const avatarSrc = safeUrl(item.thumbnailUrl);
  ...
  return (
    ...
    ">
      ...
      <div className="flex flex-col justify-center max-w-full sm:max-w-[440px] h-[120px] sm:h-[130px] overflow-hidden">
        ...
        />
        <p className="text-[13px] sm:text-[14px] lg:text-[16px] line-clamp-5 text-[#333]">
      ...
      <div className="flex flex-row sm:flex-col items-center gap-[10px]">
        {hasAvatar ? (
          ...
          <div
            style={{
              ...
              height: "40px",
              borderRadius: "50%",
              background: "linear-gradient(135deg, #00B8C6 0%, #0097A7 100%)",
              display: "flex",
              alignItems: "center",
        ...
        <div className="text-center">
          <p className="text-[12px] font-semibold text-[#222]">{item.name}</p>
