...
import navData from "@/shared/data/navbar.json";
import type { NavbarData } from "@/shared/types/navbar";
...
export default function Navbar() {
  ...
  useEffect(() => {
    fetch(`${BACKEND}/api/course-categories/with-courses`, { cache: "no-store" })
      .then(r => r.ok ? r.json() : [])
      .then((d: CategoryWithCourses[]) =>
        Array.isArray(d) && setCategories(d.filter(cat => cat.courses.length > 0))
      )
      .catch(() => {});
  }, []);
