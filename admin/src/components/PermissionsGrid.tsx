...
export default function PermissionsGrid() {
  ...
  const modal = showModal && mounted ? createPortal(
    ...
    >
      ...
      {/* Modal panel */}
      <div
        className="relative z-[60] flex flex-col bg-white border border-slate-200 shadow-2xl rounded-2xl overflow-hidden w-full max-w-xl mx-6"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 h-11 border-b border-slate-100 bg-white flex-shrink-0">
          <div className="flex items-center gap-2.5 min-w-0">
            <span className="w-[3px] h-4 rounded-full bg-[#00B8C6] shrink-0" />
            <ShieldCheck className="w-3.5 h-3.5 text-[#00B8C6] shrink-0" />
            <h2 className="text-[12.5px] font-bold text-slate-800 shrink-0">
          ...
          {isLoading ? (
            ...
            <div className="divide-y divide-slate-100">
              {MODULES.map((mod) => {
                ...
                return (
                  <div key={mod.key} className="flex items-center gap-4 py-2.5 first:pt-1 last:pb-1">
                    {/* Module label */}
                      ...
                      {ops.map((op) => {
                        ...
                        return (
                          <button
                            ...
                            className={[
                              'h-6 px-2.5 rounded-full text-[10px] font-semibold border transition-all select-none leading-none',
                              active
        ...
        {/* Footer */}
        {!isAdmin && !isLoading && (
          <div className="px-5 h-11 border-t border-slate-100 flex items-center justify-end gap-2 bg-slate-50/60 flex-shrink-0">
            <button
              type="button"
              onClick={() => setShowModal(false)}
