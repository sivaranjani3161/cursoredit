...
export default function NestedEntityManager({ title, items, onChange, showIcon = false, numberField }: Props) {
  ...
  return (
    <div className="space-y-2">
      ...
      {items.length === 0 ? (
        <div className="py-6 border-2 border-dashed border-slate-300 rounded-xl text-center text-xs text-slate-500">
        ...
        <div className="space-y-2">
          {items.map((item, index) => (
            <div key={index}
              className="group flex gap-2 bg-white border border-slate-300 rounded-xl p-3 hover:border-[#00B8C6]/40 transition-all">
              <div className="flex-shrink-0 flex items-start pt-1 text-slate-400">
              ...
              <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="space-y-2">
                  <div className="flex gap-2">
                    {numberField && (
                      ...
                      <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1">{numberField.label}</label>
                    ...
                    <div className="flex-1">
                      <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1">Title</label>
                  ...
                  {showIcon && (
                    <div className="flex items-center gap-2">
                      ...
                      {item.icon ? (
                        <div className="relative w-8 h-8 rounded-lg border border-slate-300 overflow-hidden bg-slate-50 flex-shrink-0">
                        ...
                        <button type="button" disabled={uploadingIdx === index}
                          ...
                          className="w-8 h-8 rounded-lg border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-500 hover:border-[#00B8C6]/70 hover:text-[#00B8C6] transition-all">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Bullet Points</label>
                    ...
                    {(item.description || []).map((point, pIdx) => (
                      <div key={pIdx} className="flex items-center gap-1.5">
                        ...
                        <input type="text" value={point} placeholder="Bullet point..."
                          ...
                          className="flex-1 bg-slate-50 border border-slate-100 focus:border-slate-300 text-[11px] py-1 px-2 rounded-md text-slate-800 placeholder:text-slate-400 outline-none transition-all" />
                        <button type="button" onClick={() => updateItem(index, 'description', (item.description || []).filter((_, i) => i !== pIdx))}
                          className="text-slate-400 hover:text-rose-500 transition-colors"><X className="w-3 h-3" /></button>
                      </div>
                    ))}
                    {!(item.description?.length) && <p className="text-[10px] text-slate-400 italic">No points added</p>}
                  </div>
                </div>
              </div>
